...
layout(location = 16) in vec3 ambient;//doesn't seem to be working, set to 0.2f manually
...
void main() {
	...
	//for(int i = 0;i<4;i++){		
		...
		totalDiffuse = totalDiffuse + (max(dot(N,L),0.0) * fragColor * lightColour[i],0.0);
